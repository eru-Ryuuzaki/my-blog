...
```
css预处理器是用一种专门的语言，进行网页的样式设计，之后在被编译为正常的css文件，以供项目使用。使用css预处理语言的好处：是css更加简洁、方便修改、可读性强、适应性强并且更易于代码的维护。
```
...
两者是有不同的，继sass之后scss的编写规范基本和css一致，sass时代是有严格的缩进规范并且有‘{}’和‘；’。
而scss则和css的规范是一致的
```
...
![](images/微信截图_20200831202622.png)

- **选择安装目录及选项**

![](images/微信截图_20200831202830.png)

- **选择组件，默认即可**

![](images/微信截图_20200831202927.png)

- **安装中**

![](images/微信截图_20200831203048.png)

- **结束安装**

![](images/微信截图_20200831203340.png)

**窗口自动弹出如下内容，然后输入3**

![](images/微信截图_20200831203441.png)

## Ruby测试

打开命令行，查看Ruby的版本号，如果成功就安装成功了

![](images/微信截图_20200831203801.png)
...
如上已经安装成功。但因为国内网络的问题导致`gem`源间歇性中断因此我们需要更换`gem`源。

请尽可能用比较新的 RubyGems 版本，建议 2.6.x 以上。

```shell
# 更新RubyGems (如果安装的是最新版本,不需要执行这句话)
gem update --system 
...
![](images/微信截图_20200831205111.png)

![](images/微信截图_20200831205247.png)

## sass测试

安装完成之后，你应该通过运行下面的命令来确认应用已经正确地安装到了电脑中：

![](images/微信截图_20200831205359.png)
...
```html
https://www.cnblogs.com/Full--Stack/p/8041289.html
```
...
这里推荐[koala](https://www.sass.hk/skill/koala-app.html),它是优秀的编译器，界面清晰简洁，操作起来也非常简单。鉴于koala是免费编译器，简单操作如下图:

![](images/微信截图_20200831212039.png)

koala是一个国产免费前端预处理器语言图形编译工具，支持Less、Sass、Compass、CoffeeScript，帮助web开发者更高效地使用它们进行开发。跨平台运行，完美兼容windows、linux、mac。

## Live Sass编译器

![](images/微信截图_20200831212637.png)

VSCode扩展，可通过实时浏览器重新加载来帮助您实时将SASS / SCSS文件编译/转换为CSS文件。

- `Live Sass Compiler`使用的人多，依赖于`Live Server`，所以安装时会如果没有`Live Server`会自动安装。
- 安装完成点击vscode底部的 `watch Sass` 来启动实时编译。
- [sass中文网：live sass Compiler](https://www.sass.hk/skill/sass154.html)
- github：常用设置
  - 可以设置编译的四种css风格、后缀名、保存位置
  - 可以设置 编译 排除指定文件夹，或排除指定文件外的文件
  - 或设置只编译指定文件
  - 可以设置编译的后缀名，[小程序用的话就可以编译后缀名为wxss](https://www.jianshu.com/p/fe11340ebd75)
  - 等等

### 使用技巧

点击`watch Sass`的时候，会自动把你打开的文件夹中的.sass和.scss文件全都编译，因为有这个默认设置

![](images/微信截图_20200831212741.png)

![](images/微信截图_20200831212758.png)

- 但是相信大部分人大多数时候是想只让他编译当前正在操作的文件
...
![ ](images/1-200225141513942.gif)

## EASY SASS

![](images/微信截图_20200831213141.png)

这款比较轻量，唯一缺点是不能生成css.map文件

- 虽然下载的人比live sass少，但如果不需要css.map文件的话，还是很好用的。
- 只编译当前修改并执行保存的sass文件
- 常见设置[https://blog.csdn.net/yang450712123/article/details/88869253/https://www.cnblogs.com/ldlx-mars/p/9242155.html]
  - 保存文件自动编译
  - 可以设置编译的四种css风格、后缀名、保存位置
  - 可以设置 编译 排除指定文件夹
  - 等等
...
![](images/微信截图_20200901200240.png)
...
8-1节介绍了[混合器](https://www.sass.hk/guide/)主要用于展示性样式的重用，而类名用于语义化样式的重用。因为继承是基于类的（有时是基于其他类型的选择器），所以继承应该是建立在语义化的关系上。当一个元素拥有的类（比如说`.seriousError`）表明它属于另一个类（比如说`.error`），这时使用继承再合适不过了。
